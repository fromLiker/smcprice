pipeline {
  ...
  stages {
    ...
    stage('docker build & push & run') {
      ...
      steps {
        script {
          def REMOVE_FLAG_C = sh(returnStdout: true, script: "docker container ls -q --filter name=.*SMC-Price.*") != ""
          echo "REMOVE_FLAG_C: ${REMOVE_FLAG_C}"
          if(REMOVE_FLAG_C){
            sh 'docker container rm -f $(docker container ls -q --filter name=.*SMC-Price.*)'
          }
        ...
        withCredentials([usernamePassword(credentialsId: 'liker163ID', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
          ...
          sh 'docker run -d -p 8752:8752 --memory=600M --network smc-net --name SMC-Price ${DOCKERHUBNAME}/price'
        }
      }
    }
